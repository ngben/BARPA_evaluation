...
def monthly_mean(trial,year,sim,varlist):
    ...
    for mon in range(1,13):
      ...
      for var in varlist:
        print(var)
        data=iris.load((path+file_p).format(trial=trial,yearmon=yearmon,sim=sim,var=var,time=time,owner=owner),ct)
        iris.util.equalise_attributes(data)
        try:
            data=data[:].concatenate_cube()
        except iris.exceptions.ConcatenateError:
            data=data[:].merge_cube()
        mean = data.collapsed(["time","longitude"],iris.analysis.MEAN)
        out += [mean]
    iris.util.equalise_attributes(out)
    out=out.merge()
    iris.save(out,outdir.format(trial=trial,year=year),zlib=True)

outdir = "/scratch/tp28/eh6215/mon_pl/{trial}_{year}_xsection.nc"
