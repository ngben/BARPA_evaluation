...
varlist = {'prcp':['av_prcp_rate','SLV1H',iris.analysis.MEAN],
        'Tmax':["temp_scrn",'SLV15M',iris.analysis.MAX],
        'Tmin':["temp_scrn",'SLV15M',iris.analysis.MIN],
        'latent':['av_lat_hflx','SLV1H',iris.analysis.MEAN],
        'sensible':['av_sens_hflx','SLV1H',iris.analysis.MEAN],
        'vapour_pressure_pm':[[vapour_pressure,'qsair_scrn','sfc_pres'],'SLV1*',iris.Constraint(time=lambda t: t.point.hour==6 and t.point.minute==0)],
        'int_q':['ttl_col_q','SLV1H',iris.analysis.MEAN],
        'max_wind':['max_wndgust10m','SLV15M',iris.analysis.MAX],
        'mean_wind':[[wind_speed,'uwnd10m','vwnd10m'],'SLV15M',iris.analysis.MEAN],
...
#        'low_cloud':['low_cld','SLV1H',iris.analysis.MEAN],
#        'mid_cloud':['mid_cld','SLV1H',iris.analysis.MEAN],
#        'high_cloud':['hi_cld','SLV1H',iris.analysis.MEAN]}
        }
    ...
    for mon in range(1,13):
      ...
      for var in varlist:
        ...
        if type(name) is list:
          ...
          if type(agg)==iris.Constraint:
            data = data.extract(agg)
        ...
        if type(agg)==iris.Constraint:
            ...
            data=data.aggregated_by('doyr',agg)
        ...
        data.rename(var)
        out.append(data.collapsed('time',iris.analysis.MEAN))
    iris.util.equalise_attributes(out)
    out = out.merge()
    iris.save(out,outdir.format(trial=trial,year=year),zlib=True)

outdir = "/scratch/tp28/eh6215/monthly/{trial}/{year}_means.nc"
owner='chs548'
