...
def main(exp,year):
  ...
  for mon in range(1,13):
    ...
    for var,stream in [("av_prcp_rate","SLV1H"),("ttl_col_q","SLV1H"),("av_olr","SLV1H"),("mslp","SLV15M")]:
  ...
  iris.util.equalise_attributes(data)
  data=data.merge()
  iris.save(data,outpath+exp+"%04d.nc"%year,zlib=True)
  for f in files:
    os.remove(f)
...
for i,exp in enumerate(runs):
  if i==4:
    for year in range(runs[exp][1],runs[exp][2]):
        if not os.path.exists(outpath+exp+"%04d.nc"%year):
            main(exp,year)
