...
def load_aggregate(var,path,years,constraints,aggregators,name=None):
    data= iris.load([os.path.join(path,str(year),"*.nc") for year in years])#,constraints,callback).extract(var)
...
def main(exp):
    files = []
    path = "/g/data/ia39/aus-ref-clim-data-nci/gpcp/data/mon/v2-3/"
    ...
    for var in ['NOAA Climate Data Record (CDR) of GPCP Monthly Satellite-Gauge Combined Precipitation']:
        files.append(load_aggregate(var,path,np.arange(1996,2022),cx&cy,[('aggregate','month',iris.analysis.MEAN)]))
