...
def get_dictionary(key):
    ...
    asop_dict = {}
    if key == 'BARPAR':
        asop_dict['infile']       = '/g/data/tp28/BARPA/trials/BARPA-EASTAUS_12km/era/erai/historical/r0/pp_unified/daily/pr/0p11deg/{year}/pr_BARPA-EASTAUS_12km_erai_historical_r0_BARPA_daily_{year}{month:02d}*.nc'
        ...
        asop_dict['infile']       = '/g/data/tp28/BARPA/trials/BARPAC-T_km4p4/era/erai/historical/r0/pp_unified/daily/pr/0p04deg/{year}/pr_BARPAC-T_km4p4_erai_historical_r0_BARPAC_daily_{year}{month:02d}*.nc'
...
def calc(dataset,yearmons):
    ...
    # Allocate memory for multi-model fields
        ...
        precip = asop.read_precip(asop_dict,yearmons)
...
def plot(datasets,years):
    ...
    for i in xrange(n_datasets):
        ...
        (oned_hist, twod_hist, corr_map, lag_vs_distance, autocorr, npts_map, npts, all_distance_correlations[i],all_distance_ranges[i],all_distance_max[i],all_time_correlations[i],all_time_max[i],space_inter, time_inter)=pickle.load(open("/scratch/tp28/eh6215/asop/asop_coherence_%s_%d.pickle"%(dataset,years[0]),'rb'))
